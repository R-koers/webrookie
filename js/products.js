...
async function loadProducts() {
    try {
        ...
        if (cachedProducts && cacheAge < 3600000) {
            ...
            // LET OP: geen return hier — we tonen snel de cache maar proberen alsnog de server te vragen
        } else if (cachedProducts && cacheTimestamp) {
            // Er is cache, maar te oud — we kunnen hem alvast tonen terwijl we vernieuwen
            try {
                allProducts = JSON.parse(cachedProducts);
                filteredProducts = [...allProducts];
                console.log('Oude cache gebruikt terwijl servercontrole plaatsvindt:', allProducts.length);
            } catch (e) {
                // parse error: negeer cache
                console.warn('Kon cached products niet parsen, negeer cache.', e);
            }
        }

        // Als we hier zijn: probeer altijd de server (Netlify root) — cache-buster zodat updates meteen binnenkomen
        console.log('Producten laden van server...');
        let serverData = null;
        try {
            const response = await fetch('/products.json?' + Date.now(), { cache: 'no-cache' });
            if (response.ok) {
                const data = await response.json();
                serverData = data;
                console.log('Products geladen vanaf server');
            } else {
                console.warn('Server response niet OK:', response.status);
            }
        } catch (e) {
            console.warn('Fetch naar server mislukt:', e);
        }

        // Als serverData succesvol is, gebruiken we die en updaten we localStorage
        if (serverData) {
            allProducts = serverData.components || (Array.isArray(serverData) ? serverData : []);
            filteredProducts = [...allProducts];
            ...
            console.log('Producten vernieuwd vanaf server en opgeslagen in localStorage:', allProducts.length);
            return allProducts;
        }

        // Als server niet beschikbaar is maar we hebben cache (al ingeladen hoger), return cache
        const fallbackCache = localStorage.getItem('products');
        if (fallbackCache) {
            try {
                allProducts = JSON.parse(fallbackCache);
                filteredProducts = [...allProducts];
                console.log('Fallback: Producten geladen uit localStorage (oude data):', allProducts.length);
                return allProducts;
            } catch (e) {
                console.warn('Kon fallback cache niet parsen:', e);
            }
        }

        // Niets werkte
        console.warn('Geen products.json gevonden van server en geen geldige cache. Geef lege lijst terug.');
        return [];
        ...
        if (cachedProducts) {
            try {
                ...
                console.warn('Kon cached products niet parsen in catch:', e);
            }
        }
...
async function refreshProducts() {
    try {
        ...
        allProducts = data.components || (Array.isArray(data) ? data : []);
        filteredProducts = [...allProducts];
...
function displayProducts(products, containerId, limit = null) {
    ...
    if (!container) {
        ...
        // Zorg dat loading verborgen wordt zodat spinner niet eeuwig blijft staan
...
function getProductCategory(product) {
    ...
    if (name.includes('gpu') || name.includes('graphics') || specs.includes('gpu') || specs.includes('graphics')) {
        ...
        return 'cooling';
    } else if (name.includes('motherboard') || specs.includes('motherboard')) {
        return 'motherboard';
...
function filterAndSortProducts(category = '', sortBy = 'name-asc', searchTerm = '') {
    ...
    switch (sortBy) {
        ...
        case 'price-desc':
            ...
            break;
        default:
            // No sorting if sortBy is unrecognized
            break;
    }
...
// Helper: maak of toon een vaste "Producten verversen" knop op de parts pagina
function ensureRefreshButton() {
    // Als knop al bestaat, niks doen
    if (document.getElementById('refresh-products-btn')) return;

    const btn = document.createElement('button');
    btn.id = 'refresh-products-btn';
    ...
    btn.onclick = async () => {
        ...
        try {
            ...
            console.error('Verversen mislukte:', e);
            if (typeof window.notify === 'function') {
                window.notify('Kon producten niet verversen', 'error');
            }
        } finally {
    ...
    // Probeer de knop direct na het filter-gedeelte te plaatsen (als aanwezig)
    const searchInput = document.getElementById('search-input');
    let insertAfterNode = null;
    if (searchInput) {
        // zoek het wrapper-element van de filters (closest parent met padding)
        insertAfterNode = searchInput.closest('.bg-white') || searchInput.parentNode;
    }

    // fallback: plaats na card-container wrapper
    const cardContainer = document.getElementById('card-container');
    if (!insertAfterNode && cardContainer) {
        insertAfterNode = cardContainer.parentNode;
    }

    if (insertAfterNode && insertAfterNode.parentNode) {
        insertAfterNode.parentNode.insertBefore(btn, insertAfterNode.nextSibling);
    } else {
        // fallback: append to body onderaan main
...
async function initializeProducts() {
    ...
    // Zorg dat de loading indicator niet eeuwig blijft staan (altijd verbergen na load attempt)
    if (loadingEl) loadingEl.style.display = 'none';
    ...
    if (currentPage === 'index.html' || currentPage === '') {
        ...
        // Zorg dat er altijd een duidelijke refresh knop beschikbaar is
        ensureRefreshButton();

        // Ververs-knop toevoegen als laadfout optreedt of leeg resultaat (extra fallback)
        ...
        if (container && (!allProducts || allProducts.length === 0)) {
            ...
            // Voeg alleen toe als hij nog niet bestaat
            if (!document.getElementById('fallback-refresh-btn')) {
                btn.id = 'fallback-refresh-btn';
